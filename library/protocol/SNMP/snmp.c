...
// LED Control ///////////////////////////////////////////////////////////////////////////
int32 wiznetLedStatus = WIZ_LED2;

void getWIZnetLed(void *ptr, uint8 *len)
{
	if ( wiznetLedStatus==0 )	*len = sprintf((char *)ptr, "LED Off");
	else						*len = sprintf((char *)ptr, "LED On");
}
...
void UserSnmpDemo()
{
...

	{
        ...
        SnmpXTrapSend("222.98.173.214", "222.98.173.213", "public", enterprise_oid, 1, 0, 0);
		SnmpXTrapSend("222.98.173.214", "127.0.0.0", "public", enterprise_oid, 6, 0, 2, &trap_oid1, &trap_oid2);
	}
	...
	case SNMPDTYPE_OBJ_ID :
		{
			...
			{
				snmpData[id].dataLen = (uint8)strlen((char const*)&snmpData[id].u.octetstring);
			}
...
void ipToByteArray(int8 *ip, uint8 *pDes)
{
	...
	int8 buff[32];
	uint32 len = (uint32)strlen((char const*)ip);
	...
	case SNMPDTYPE_OBJ_ID :
		{
			...
			{
				oid_data->dataLen = (uint8)strlen((char const*)&oid_data->u.octetstring);
			}
...
int32 SnmpXTrapSend(int8* managerIP, int8* agentIP, int8* community, dataEntryType enterprise_oid, uint32 genericTrap, uint32 specificTrap, uint32 va_count, ...)
{
	...
	packet_trap[packet_index++] = (uint8)strlen((char const*)community);
	memcpy(&(packet_trap[packet_index]), community, strlen((char const*)community));

	packet_index = packet_index + (uint8)strlen((char const*)community);
	...
	packet_trap[packet_buff2] = packet_index - (9 + (uint8)strlen((char const*)community));
	...
	// Send Packet
	{
		...
		UDPSend(SOCK_SNMP, (int8*)packet_trap, packet_index, svr_addr, 162);
