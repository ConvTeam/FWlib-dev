...
int32 SnmpXTrapSend(int8* managerIP, int8* agentIP, int8* community, dataEntryType enterprise_oid, uint32 genericTrap, uint32 specificTrap, uint32 va_count, ...)
{
	...
	// Send Packet
	{
		...
		int32 trapfd=0, slen=0;
		slen = sizeof(si_other);

		if ( (trapfd=(int32)socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP))==-1 )
		{
			return 1;
		}
		...
		si_other.sin_port = htons(162);
		si_other.sin_addr.s_addr = inet_addr(managerIP);

		sendto(trapfd, packet_trap, packet_index, 0, (struct sockaddr *)&si_other, slen);
		closesocket(trapfd);
#else
		uint8 svr_addr[6];
		...
		UDPSend(SOCK_SNMP, packet_trap, packet_index, svr_addr, 162);
...
int32 SnmpXDaemon()
{
...
#ifdef WIN32
	snmpfd = (int32)socket(AF_INET, SOCK_DGRAM, 0);
	...
	if (retStatus < 0)
	{
		WDEBUG("Unable to bind to socket.");
		return 1;
	}
#else
	uint8 svr_addr[6];
	...
	while(loopsnmpd)
	{
#ifdef WIN32          
		fromlen = sizeof(from);
		request_msg.len = recvfrom(snmpfd, &request_msg.buffer[0], 1024, 0, &from, &fromlen);
		WDEBUG("Received datagram from %s, port %d", inet_ntoa(((struct sockaddr_in *)&from)->sin_addr), ((struct sockaddr_in *)&from)->sin_port);
#else
		...
		if (request_msg.len > 0)
		{
			...
			{
#ifdef WIN32				
				sendto(snmpfd, response_msg.buffer, response_msg.index, 0, (struct sockaddr *)&from, fromlen);
#else
